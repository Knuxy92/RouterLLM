...
- **Admin console**: set `ROUTERLLM_ADMIN_TOKEN` in `.env` to enable the embedded React console at `/admin/` and its API at `/admin/api`. The Docker Compose config mounts `routerllm.yaml` read-write because provider/route toggles and leg edits are persisted surgically to the file without expanding `${ENV_VAR}` placeholders. Without the token, admin endpoints return 403. Every write also drops a `.bak` next to the file (gitignored), and the image chowns `/app` to UID 65532 so the non-root user can create both the backup and the atomic temp file.
