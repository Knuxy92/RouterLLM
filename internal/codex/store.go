...
func DefaultAccountsPath() string {
	...
	if err != nil {
		return defaultAccountsFile
	}

	return filepath.Join(filepath.Dir(exe), defaultAccountsFile)
}
...
func LoadAccountStore(path string) (*AccountStore, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read codex accounts %q: %w", path, err)
	}

	var file accountsFile
	if err := json.Unmarshal(raw, &file); err != nil {
		return nil, fmt.Errorf("parse codex accounts %q: %w", path, err)
	}
...
// first so an account added by another process (a concurrent codex login)
...
func (s *AccountStore) mergeFileLocked() error {
	...
	if err != nil {
		return fmt.Errorf("read codex accounts %q: %w", s.path, err)
	}

	var file accountsFile
	if err := json.Unmarshal(raw, &file); err != nil {
		return fmt.Errorf("parse codex accounts %q: %w", s.path, err)
	}
...
func (s *AccountStore) persist() error {
	...
	if err != nil {
		return fmt.Errorf("encode codex accounts: %w", err)
	}
...
func writeFileAtomic(path string, data []byte, perm os.FileMode) error {
	...
	if err != nil {
		return fmt.Errorf("write codex accounts %q: %w", path, err)
	}
